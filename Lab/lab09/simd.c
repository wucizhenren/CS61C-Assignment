...
long long int sum_simd(int vals[NUM_ELEMS]) {
  ...
  for (unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
    ...
    for (unsigned int i = 0; i < NUM_ELEMS / 4 * 4; i += 4) {
      ...
      vals_vec = _mm_and_si128(vals_vec, _mm_cmpgt_epi32(vals_vec, _127));
      sum_vec = _mm_add_epi32(sum_vec, vals_vec);
    }
    int sums[4];
...
long long int sum_simd_unrolled(int vals[NUM_ELEMS]) {
  ...
  for (unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
    ...
    for (unsigned int i = 0; i < NUM_ELEMS / 16 * 16; i += 16) {
      ...
      vals_vec = _mm_and_si128(vals_vec, _mm_cmpgt_epi32(vals_vec, _127));
      sum_vec = _mm_add_epi32(sum_vec, vals_vec);
      vals_vec = _mm_loadu_si128((__m128i *)&vals[i + 4]);
      vals_vec = _mm_and_si128(vals_vec, _mm_cmpgt_epi32(vals_vec, _127));
      sum_vec = _mm_add_epi32(sum_vec, vals_vec);
      vals_vec = _mm_loadu_si128((__m128i *)&vals[i + 8]);
      vals_vec = _mm_and_si128(vals_vec, _mm_cmpgt_epi32(vals_vec, _127));
      sum_vec = _mm_add_epi32(sum_vec, vals_vec);
      vals_vec = _mm_loadu_si128((__m128i *)&vals[i + 12]);
      vals_vec = _mm_and_si128(vals_vec, _mm_cmpgt_epi32(vals_vec, _127));
      sum_vec = _mm_add_epi32(sum_vec, vals_vec);
    }
