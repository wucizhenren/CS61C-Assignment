...
#pragma omp parallel
  {
    ...
    for (int i = omp_get_thread_num(); i < ARRAY_SIZE; i += ThreadNum)
      z[i] = x[i] + y[i];
  }
}
...
#pragma omp parallel
  {
    ...
    int boundBegin = ThreadID * ThreadNum;
    int boundEnd = ThreadID * ThreadNum + ThreadNum;
    if (ThreadID == ThreadNum - 1) {
      for (int i = boundBegin; i < ARRAY_SIZE; i++)
        z[i] = x[i] + y[i];
    } else {
